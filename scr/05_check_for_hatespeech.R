...
kommentare_unprocessed <- data.table::fread(here::here("data", "kommentare.csv")) %>% 
  tibble::as_tibble()
...
# "wählsch aimool linggi und aimool grieni - waisch, s`stimmt ebbys nimm im hirni!"
# c("Trottel")
# c("dubbu", "Tubel", "Spiesser")
# c("Dummheit")
## Lustig
# Ein hoher Mandel an Vitamin B12 kann Halluzinationen auslösen!

# preprocessing
kommentare_preprocessed <- kommentare_unprocessed %>%
  mutate(
    text_display = clean_text_for_bert(snippet_text_display)
  )
...
portions <- kommentare_preprocessed %>%
  select(video_id, text_display, id) %>%
  # split up 
  mutate(comment_split = strsplit(text_display, split = " ")) %>%
...
classify_and_save <- function(comment, video_id_arg, comment_id_arg, portion_arg) {
  ...
  if (!rlang::is_null(inference_output$result)) {
    ...
    data.table::fwrite(
      ...
      file = here::here(
        ...
        paste0(video_id_arg, "_", comment_id_arg, "_", portion_arg, ".csv")
      )
    )
    ...
    inference_output$error %>%
      ...
      writeLines(
        ...
        con = here::here(
          ...
          paste0(video_id_arg, "_", comment_id_arg, "_", portion_arg, ".txt")
          )
      )
  }
}
...
portions %>%
  mutate(
    classification = purrr::pwalk(
      list(comments_reportion, video_id, id, group_no),
      classify_and_save
    )
  )
...
classifier_resultate_pfade <- fs::dir_ls(
  here::here("raw", "classifier", "results")
)
classifier_resultate <- purrr::map_df(
  classifier_resultate_pfade,
  data.table::fread,
  colClasses = list(
    character = c("label", "comments_reportion", "id", "video_id"),
    double = c("group_no", "score")
  )
)
...
data.table::fwrite(
  ...
  here::here("data", "klassifiziert.csv")
)
